...
const config = {
  ...
  events: {
    ...
    move: onMove              // When user completes a move (click-to-move or drag&drop)
  },
  drawable: {
    enabled: true,
    visible: true,
...
console.log('✅ Chessground board initialized with Chess.js v1.4.0');
...
/**
 * Fetch opening analysis from backend
 * @param {string} fen - FEN string of current position
 */
async function fetchAnalysis(fen) {
  try {
    const res = await fetch(`/api/analyse?fen=${encodeURIComponent(fen)}`);
    const data = await res.json();
    // Assuming a renderStats function exists in main script
